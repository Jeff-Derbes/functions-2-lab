...
// CODE HERE
// For some reason. this doesn't return an accurate number with the provided array, but it does when i test it on other numbers
const totalPopulation = populations.reduce((pop, sum) => pop + sum);
...
// CODE HERE
const myStrongest  = monstersInYourPocket.filter(function(arr){
  return arr['CP'] > 200;
})
...
// This one was very diffficult for me. I'm not confident that this is how it is supposed to be done but I guess it works for now. I couldn't find another way to access each object in the reduce method so I used filter to get just bob's objects, then use map to put his purchases into an array, and finally used reduce to sum those values.

const getBob = purchases.filter(ownerName => ownerName.owner === "Bob")

console.log(getBob)

const bobsPurchases = getBob.map(priceValue => priceValue.price)

console.log(bobsPurchases)

const bobsTotal = bobsPurchases.reduce((sum, price)=>{
  return sum + price
})
